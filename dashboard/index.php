...
require_once("../core/connect.core.php");
require_once("../core/functions.core.php");

// Normalize request variables to avoid undefined index notices
$p = isset($_GET['p']) ? addslashes($_GET['p']) : '';
$lang = (isset($_GET['lang']) && $_GET['lang'] != '') ? addslashes($_GET['lang']) : (isset($_SESSION['lang']) ? $_SESSION['lang'] : 'th');
$_SESSION['lang'] = $lang;

// Redirect if not logged in (allow debug bypass with ?debug=1)
if(empty($_SESSION['uname']) || (isset($_SESSION['uclass']) && $_SESSION['uclass'] == 1)){
...
$ukey = isset($_SESSION['ukey']) ? $_SESSION['ukey'] : '';
$userdata = $getdata->my_sql_query(NULL,"user","user_key='".$ukey."'");
$system_info = $getdata->my_sql_query(NULL,"system_info",NULL);
date_default_timezone_set('Asia/Bangkok');
require_once("../core/online.core.php");
?>
<head>
...
<?php  
if($p == "cashier_nomember" || $p == "import" || $p == "cashier_member"){
    ...
    <div id="wrapper">
        ...
        <nav class="navbar navbar-default navbar-fixed-top" role="navigation" style="margin-bottom: 0">
            ...
            <!-- /.navbar-header -->
             ...
             <?php
               $user_photo = (is_object($userdata) && !empty($userdata->user_photo)) ? $userdata->user_photo : '';
               $user_name = (is_object($userdata) && !empty($userdata->name)) ? $userdata->name : '';
               $user_lastname = (is_object($userdata) && !empty($userdata->lastname)) ? $userdata->lastname : '';
             ?>
             <a href="?p=setting_info"><div class="box_user_right"><div class="box_img_cycle2"><img src="../resource/users/thumbs/<?php echo $user_photo;?>" <?php echo getPhotoSize('../resource/users/thumbs/'.$user_photo);?> id="img_cycle2" alt=""/></div><?php echo $user_name."&nbsp;&nbsp;&nbsp;".$user_lastname;?></div></a>
            <!-- /.navbar-top-links -->
            <div class="navbar-default sidebar" role="navigation">
              <div class="sidebar-nav navbar-collapse">
                  ...
                  while($showsubmenus = mysqli_fetch_object($getsubmenus)){
                    $getactive = $getdata->my_sql_query("menu","list","cases='".$p."'");
								  $showm .='<li><a href="'.$showsubmenus->menu_link.'" ';
                    if($p == $showsubmenus->menu_case || (is_object($getactive) && $getactive->menu == $showsubmenus->menu_case)){$showm .='class="active"';}
							  ...
							  $showm ='<li><a href="'.$showmenus->menu_link.'" ';
                              $getactive = $getdata->my_sql_query("menu","list","cases='".$p."'");
                              if($p == $showmenus->menu_case || (is_object($getactive) && $getactive->menu == $showmenus->menu_case)){$showm .='class="active"';}

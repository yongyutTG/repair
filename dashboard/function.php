...
$getdata->my_sql_set_utf8();
	switch(addslashes($_GET['type'])){
		...
		while($subtotalpoint = mysqli_fetch_object($totalpoint)){
		...
		while($subtotalcom = mysqli_fetch_object($totalcom)){
		...
		while($showpex = mysqli_fetch_object($cancel_rec)){
		...
		while($showfree = mysqli_fetch_object($cancel_free)){
		...
		while($showpim = mysqli_fetch_object($cancel_pim)){

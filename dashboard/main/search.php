...
<div class="modal fade" id="edit_status" tabindex="-1" role="dialog" aria-labelledby="memberModalLabel" aria-hidden="true">
  ...
  <div class="container-fluid">
    ...
    <div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
      ...
      <form class="navbar-form navbar-left" role="search" method="get">
        ...
        <input type="text" class="form-control  " name="q" placeholder="ระบุชื่อ/หมายเลขโทรศัพท์หรือรหัสส่งซ่อม/เคลม เพื่อค้นหา" value="<?php echo @addslashes($_GET['q']);?>" size="100">
        </div>
 ...
 <div class="table-responsive">
  ...
  <?php
	   ...
	   $getcard = $getdata->my_sql_select(NULL,"card_info"," (card_customer_name LIKE '%".addslashes($_GET['q'])."%') OR (card_customer_lastname LIKE '%".addslashes($_GET['q'])."%') OR (card_customer_phone LIKE '%".addslashes($_GET['q'])."%') OR (card_customer_email LIKE '%".addslashes($_GET['q'])."%') OR (card_code LIKE '%".addslashes($_GET['q'])."%') ORDER BY card_insert");

...
if(isset($_POST['backup'])){
	...
	while ( $row = mysqli_fetch_row($showresult) ) {
  		$table = $row[0];
	    // dump data
		$stringData = datadump($table, true, false);
...
if(isset($_POST['submit_restore'])){
	$restore_file = isset($_FILES['file_restore']) ? $_FILES['file_restore'] : null;
	if (!$restore_file || $restore_file['error'] !== UPLOAD_ERR_OK || !is_uploaded_file($restore_file['tmp_name'])) {
	...
	} elseif (strtolower(pathinfo($restore_file['name'], PATHINFO_EXTENSION)) !== 'sql') {
		...
		if ($restore_obj->Execute($restore_file['tmp_name'], MSR_FILE, false, false)) {
			...
			$restore_error = htmlspecialchars($restore_obj->error, ENT_QUOTES, 'UTF-8');
			$alert = '<div class="alert alert-danger alert-dismissable"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>กู้คืนข้อมูลไม่สำเร็จ: '.nl2br($restore_error).'</div>';
		}
	}
}
echo @$alert;
